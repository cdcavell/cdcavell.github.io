...
Interesting scenario I faced recently, develop a process that authenticates a user; check for an existing registration; and if not found, redirect to registration process. I solved this with a combination of `OpenIdConnect`{:.language-csharp .highlight} `OnTicketReceived`{:.language-csharp .highlight} event handler along with [`Authorization Policies`](https://docs.microsoft.com/en-us/aspnet/core/security/authorization/policies?view=aspnetcore-3.1).

In `Startup.cs`{:.language-csharp .highlight} `ConfigureServices`{:.language-csharp .highlight} method, I have a call to the `OnTicketRecieved`{:.language-csharp .highlight} handler in the Events option of the `AddOpenIdConnect`{:.language-csharp .highlight} option of `services.AddAuthentication`. This allows me to inspect the `Identity`{:.language-csharp .highlight} `Claims`{:.language-csharp .highlight} sent from IdP and add additional claims to `Identity`{:.language-csharp .highlight} before further pipeline processing. I am able to get the `dbContext`{:.language-csharp .highlight} from `HttpContext`{:.language-csharp .highlight} and perform a lookup to check for existing registration record based on retrieved email claim type. If found I set a custom claim type “_registration_” to “_existing_” otherwise I set to “_new_”. This custom claim type is then added to `Identity`{:.language-csharp .highlight} `Claims`{:.language-csharp .highlight} and `Response`{:.language-csharp .highlight} is sent on through pipeline for further processing.
...
I also register a “_NewRegistration_” and “_ExistingRegistration_” policy in `Startup.cs`{:.language-csharp .highlight} `ConfigureServices`{:.language-csharp .highlight} as well as the corresponding handler classes for direct injection.
...
If `Identity`{:.language-csharp .highlight} follows the normal process flow of application, it will access the _Login Action_ of _Account Controller_. 
...
_Login Action_ will check if `Identity`{:.language-csharp .highlight} has a "_NewRegistration_" policy and if so redirects to registration process otherwise redirects to _Index Action_ of _Home Controller_. This check is performed by calling `AuthorizeAsync`{:.language-csharp .highlight} of the `IAuthorizationService`{:.language-csharp .highlight} interface direct injected in constructor method of _Account Controller_.
...
Modified registration of authorization handlers in `Startup.cs`{:.language-csharp .highlight} `ConfigureServices`{:.language-csharp .highlight} method from Singleton to Transient in order to direct inject dbContext to authorization handlers.
{% highlight csharp %}
        // Registered authorization handlers
